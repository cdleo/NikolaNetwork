...
Subscribe::Subscribe(CapabilityDefinition capability, std::string userHash) 
: Request(CommandID_Subscribe), capability_(capability), userHash_(userHash) {
    
}

Subscribe::Subscribe(Subscribe* other)
: Request(CommandID_Subscribe), capability_(other->getCapability()), userHash_(other->getUserHash()) {
}

std::string Subscribe::encode() {
...
bool Subscribe::decode(const void* buffer, size_t len) {
    ...
    LOG->LogMessage(LOG_DEBUG,"Subscribe::decode(): Capability type=%d id=%d", (int)capability_.getType(), capability_.getId());
    return true;
}

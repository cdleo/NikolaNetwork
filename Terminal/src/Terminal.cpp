...
Terminal::Terminal(std::string routeToConfigFile) 
 : exitRequired_(false) {
    ...
    if (!CONN_HANDLERS->initialize())
        LOG->WARNING("[Terminal::error] Unable to initialize connections! Exitting!");
}
...
void Terminal::initializeSignalsManager(){
    std::vector<int> signals;
    signals.push_back(SIGINT);
    SIGNALS->initialize(this, signals);
}

void Terminal::loadParameters(std::string routeToConfigFile){
...
void Terminal::onSignalTrapped(int signal_number){
    exitRequired_ = true;
}
